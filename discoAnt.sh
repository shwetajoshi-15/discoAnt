...
source discoAnt_params_"$GENE".txt
...
echo "FLAIR - correct and collapse"
	...
	python $PROGRAMS/flair/flair.py collapse -r $ALL/all.fa -q $DISCOANT/$GENE/flair/"$GENE"_all_corrected.psl -f $REF_HG38/gencode.v35.annotation.gtf -g $REF_HG38/GRCh38.p13.genome_edit.fa -o $DISCOANT/$GENE/flair/"$GENE"_collapse_500 --temp_dir $DISCOANT/$GENE/flair -s 500
...
##########                                         ##########
########## 8. Quantification based on the metagene ##########
##########                                         ##########

echo "Quantifying transcripts based on primary alignments to the metagene"

	if [[ ! -f $DISCOANT/$GENE/flair_metagene_minimap2/"$GENE"_quant.COMPLETED ]]
	then
	...
	touch $DISCOANT/$GENE/flair_metagene_minimap2/"$GENE"_quant.COMPLETED

	else
	echo "Transcript quantification is present in $DISCOANT/$GENE/sqanti3"
	fi
