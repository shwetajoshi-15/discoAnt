...
if [ "$downsampling" == FALSE ] && [ "$rna_spike_in" == TRUE ];
then

echo "Aligning pass reads to a reference SIRVome"
	...
	minimap2 -ax splice --splice-flank=no $REF_HG38/SIRV_isoforms_multi-fasta_170612a.fasta $FASTA/${base}.fa > $RESULTS/"$GENE"/minimap2/${base}.sam
...
if [ "$downsampling"  == TRUE ] && [ "$rna_spike_in" == TRUE ];
then

echo "Aligning downsampled pass reads to a reference SIRVome"
	...
	minimap2 -ax splice --splice-flank=no $REF_HG38/SIRV_isoforms_multi-fasta_170612a.fasta $FASTA/$reads_per_barcode_post_downsampling/${base}.fa > $RESULTS/"$GENE"/minimap2/${base}.sam
	samtools view -S -h -b $RESULTS/"$GENE"/minimap2/${base}.sam | samtools sort - > $RESULTS/"$GENE"/minimap2/${base}_sorted.bam
...
if [ "$downsampling" == TRUE ] && [ "$rna_spike_in" == FALSE ];
then
...
if [ "$downsampling" == FALSE ] && [ "$rna_spike_in" == FALSE ];
then
...
if [ "$rna_spike_in" == TRUE ];
then

echo "Creating transcripts with bambu"

	Rscript $SCRIPTS/bambu_tx_discovery.R -b $RESULTS/"$GENE"/minimap2/"$GENE"_pri_merged.bam \
	-f $REF_HG38/SIRV_isoforms_multi-fasta_170612a.fasta \
	-t $REF_HG38/SIRV_isoforms_multi-fasta-annotation_C_170612a.gtf \
	-r $readFraction_for_bambu \
	-o $RESULTS/"$GENE"/bambu
...
if [ "$primer_site_based_filter" == TRUE ] && [ "$rna_spike_in" == FALSE ];
then
...
if [ "$primer_site_based_filter" == TRUE ] && [ "$rna_spike_in" == TRUE ];
then
...
echo "Creating a metatranscriptome based on the filtered transcripts"
	...
	gffread -w $RESULTS/"$GENE"/filtered_transcripts/extended_annotations_filtered.fa -g $REF_HG38/SIRV_isoforms_multi-fasta_170612a.fasta $RESULTS/"$GENE"/filtered_transcripts/extended_annotations_filtered.gtf
	salmon index -t $RESULTS/"$GENE"/filtered_transcripts/extended_annotations_filtered.fa -i $RESULTS/"$GENE"/filtered_transcripts/extended_annotations_filtered -k 31
...
echo "Creating a metatranscriptome based on the filtered transcripts"
	...
	gffread -w $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts.fa -g $REF_HG38/SIRV_isoforms_multi-fasta_170612a.fasta $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts.gtf
	salmon index -t $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts.fa -i $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts -k 31

echo "Creating metrics for the report"

	filtered_transcripts_known=$( cat $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts.txt | grep "$GENE" | grep -v _ | wc -l )
	filtered_transcripts_novel=$( cat $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts.txt | grep tx | grep -v _ | wc -l )
...
gffcompare -r $REF_HG38/SIRV_isoforms_multi-fasta-annotation_C_170612a.gtf \
-o $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts_gffcomp $RESULTS/"$GENE"/filtered_transcripts/filtered_transcripts.gtf

echo "SQANTI annotation not available for RNA spike-ins."
